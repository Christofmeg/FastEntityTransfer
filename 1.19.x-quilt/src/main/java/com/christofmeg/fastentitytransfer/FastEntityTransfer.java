    ...
    public void onInitialize(ModContainer mod) {
        ...
        AttackBlockCallback.EVENT.register((player, level, hand, pos, direction) -> {
            if (CommonClickInteractions.isCtrlKeyDown) {
                InteractionResult result = CommonClickInteractions.onLeftClickBlock(player, level, hand, pos, direction);
        ...
        UseBlockCallback.EVENT.register((player, level, hand, blockHitResult) -> {
            if (CommonClickInteractions.isCtrlKeyDown) {
                InteractionResult result = CommonClickInteractions.onRightClickBlock(player, level, hand, blockHitResult);
                if (result == InteractionResult.CONSUME) {
                    CommonClickInteractions.isCtrlKeyDown = false;
                    return result;
                }
