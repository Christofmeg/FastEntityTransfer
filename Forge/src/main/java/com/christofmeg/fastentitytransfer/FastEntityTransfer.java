...
public class FastEntityTransfer {
    ...
    public static void onLeftClickBlock(LeftClickBlock event) {
        InteractionResult result = CommonClickInteractions.onLeftClickBlock(event.getPlayer(), event.getPlayer().getLevel(), event.getHand(), event.getPos(), event.getFace(), isCtrlKeyDown);
    ...
    public static void onRightClickBlock(RightClickBlock event) {
        InteractionResult result = CommonClickInteractions.onRightClickBlock(event.getPlayer(), event.getPlayer().getLevel(), event.getHand(), event.getHitVec(), isCtrlKeyDown);
        if (result == InteractionResult.CONSUME) {
            isCtrlKeyDown = false;
            event.setCanceled(true);
        }
